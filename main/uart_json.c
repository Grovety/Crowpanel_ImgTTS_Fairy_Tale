...
static bool validate_and_parse_json_text_only(const char *json_in) {
    ...
    ESP_LOGD(TAG, "Raw JSON received: %s", copy);
    ...
    while (1) {
        ...
        if (evt.type == UART_DATA) {
            ...
            if (buffered >= UART_JSON_BUF) {
                ...
                buffered = 0;
                memset(buf, 0, UART_JSON_BUF + 1);
                continue;
            }

            buf[buffered] = '\0';

            // Parse by newline, not by timeout_flag
            while (1) {
                char *line_end = memchr(buf, '\n', buffered);
                if (!line_end) break;

                size_t line_len = (size_t)(line_end - (char*)buf) + 1;

                // Copy one line
                char *line = (char*)malloc(line_len + 1);
                if (!line) {
                    ESP_LOGE(TAG, "No memory for line");
                    break;
                }
                ...
                size_t remaining = buffered - line_len;
                memmove(buf, buf + line_len, remaining);
                buffered = remaining;
                buf[buffered] = '\0';

                // Filter: only attempt JSON parse if it looks like JSON
                // (UART0 has logs; this avoids parsing "I (...)" etc.)
                if (line[0] != '{') {
                    free(line);
                    continue;
                }

                // Optional: remove huge hexdump spam
                // ESP_LOGI(TAG, "[UART LINE] len=%u", (unsigned)strlen(line));

                if (validate_and_parse_json_text_only(line)) {
                    // Success response optional (UART0 often is console)
                    // uart_json_send_str("Success\n");
                } else {
                    // DO NOT spam Error parsing JSON into UART0
            ...
            ESP_LOGD(TAG, "UART event type: %d", evt.type);
        }
    }
