...
static bool s_tts_session_done = false;   
static bool s_tts_wait_audio_end = false; 
...
static void ui_notify_audio_finished_async(void *arg);
void ui_notify_audio_playback_finished(void);
...
static void outro_tts_timer_cb(lv_timer_t *t)
{
    ...
    if (outro) {
    s_tts_wait_audio_end = true;   
    s_tts_session_done   = false;  
...
static void ui_notify_audio_finished_async(void *arg)
{
    ...
    if (!s_tts_wait_audio_end) {
        return;
    }

    s_tts_wait_audio_end = false;
    s_tts_session_done   = false;  
...
void ui_notify_audio_playback_finished(void)
{
    lv_async_call(ui_notify_audio_finished_async, NULL);
}
